...
class DirectMessageController extends Controller
{
    ...
    public function chatList()
    {
        ...
        $subQuery = DirectMessage::select(
            DB::raw("CASE WHEN sender_id = $auth_id THEN receiver_id ELSE sender_id END as user_id"),
    ...
    {
        $authId = Auth::id();
        $direct = DirectMessage::where('receiver_id', $authId)->where('is_read', false)->count();
        $group = DB::table('group_messages')
            ->join('group_members', function ($join) use ($authId) {
                $join->on('group_members.group_id', '=', 'group_messages.group_id')
                    ->where('group_members.user_id', '=', $authId);
            })
            ->leftJoin('group_message_reads', function ($join) use ($authId) {
                $join->on('group_message_reads.group_message_id', '=', 'group_messages.id')
                    ->where('group_message_reads.user_id', '=', $authId);
            })
            ->where('group_messages.sender_id', '!=', $authId)
            ->whereNull('group_message_reads.id')
            ->count();

        return response()->json(['count' => $direct + $group]);
    }
