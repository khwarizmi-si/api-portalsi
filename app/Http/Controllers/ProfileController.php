...
class ProfileController extends Controller
{
    ...
    public function show(Request $request, $username)
    {
        ...
        if ($canViewPosts) {
            ...
            $recentPosts = $paginatedPosts->getCollection()->map(function ($post) {
                ...
                $thumbnail = $isVideo
                    ? $this->generateThumbnailUrl($post->media_url, $post->post_id)
                    : null;
    ...
    // ✅ Helper buat generate thumbnail URL
    private function generateThumbnailUrl($mediaUrl, $postId)
    {
        // contoh: ganti ekstensi jadi .jpg
        $pathInfo = pathinfo($mediaUrl);
        return url("/storage/thumbnails/{$postId}.jpg");
    }
}
